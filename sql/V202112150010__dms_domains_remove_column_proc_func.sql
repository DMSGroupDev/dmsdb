...
BEGIN
  ...
  ELSIF i_domain_name = '' OR i_domain_name IS NULL THEN
    ...
    RETURN;
  ELSE  
    INSERT INTO ${flyway:defaultSchema}.dms_domains (domain_name, active) VALUES (LOWER(i_domain_name), i_active);
    RETURN;
  END IF;
...
BEGIN
  IF i_domain_id IS NOT NULL AND i_domain_name IS NULL and i_active IS NULL THEN
  ...
  IF i_domain_id > 0 THEN
    ...
    EXECUTE v_sql USING i_domain_id, i_domain_name, i_active;
...
DROP FUNCTION IF EXISTS ${flyway:defaultSchema}.dms_domains_r(integer, character varying, integer, boolean);
...
BEGIN
  ...
  IF i_active IS NOT NULL THEN
    IF v_conditions = false THEN
      v_conditions := true;
      v_sql := v_sql || ' WHERE active = $3';
	ELSE
	  v_sql := v_sql || ' AND active = $3';
	END IF;
  END IF;

  RAISE NOTICE 'SQL: %', v_sql;
  RETURN QUERY EXECUTE v_sql USING i_domain_id, i_domain_name, i_active;
EXCEPTION WHEN OTHERS THEN
  RAISE EXCEPTION 'An unspecified error occured in procedure.';
END;
